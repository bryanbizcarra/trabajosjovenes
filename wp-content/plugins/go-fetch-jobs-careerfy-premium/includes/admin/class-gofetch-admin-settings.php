...
class GoFetch_Admin_Settings extends BC_Framework_Tabs_Page {
	 ...
	 * @var array
	 */
	protected $valid_admin_pages = array(
		'go-fetch-jobs-wpjm-settings',
		'go-fetch-jobs-wpjm-providers',
		'go-fetch-jobs-wpjm-ats',
	);
	...
	function __construct() {
		...
		add_filter( 'goft_wpjm_settings', array( $this, 'hide_provider_source_settings' ), 10, 2 );
		add_filter( 'goft_wpjm_settings', array( $this, 'hide_affiliate_param_settings' ), 10, 2 );
		add_filter( 'set-screen-option', array( $this, 'set_screen_settings' ), 15, 3 );
	...
	protected function tab_importer() {
		...
		$this->tab_sections['importer']['admin'] = array(
			...
			'fields' => array(
				...
				array(
					...
					'choices'  => $sel_roles,
					'tip'      => __( 'Choose the roles that users must have to be selectable as authors on imported jobs. Defaults to Administrators only, if none is selected.', 'gofetch-wpjm' ) .
								  '<br/><br/>' . __( 'If you have a long list of users on your site or are experiencing memory issues try limiting these roles to avoid including all users.', 'gofetch-wpjm' ),
		...
		$this->tab_sections['importer']['importer'] = array(
			...
			'fields' => array(
				...
				array(
					...
					'tip' => __( 'On feeds, that only provide job excerpts (most of them), the importer can automatically crawl the provider job page, to try to get full job descriptions.', 'gofetch-wpjm' ) .
							 '<br/><br/>' . __( 'This option only works for pre-set providers that allow scraping, it does not work on custom/unknown feeds.', 'gofetch-wpjm' ) .
							 '<br/><br/>' . __( 'Please note that scraping is resource intensive. The import process will be slower, depending on the number of jobs, you import.', 'gofetch-wpjm' ) .
							 '<br/><br/>' . __( 'Leave this option unchecked, if you prefer to enable scraping on each import, individually, or if your imports become too slow.', 'gofetch-wpjm' ),
				),
				array(
		...
		$this->tab_sections['importer']['smart_assign'] = array(
			...
			'fields' => array(
				array(
					...
					'type'  => 'checkbox',
					'desc' => __( 'Yes', 'gofetch-wpjm' ),
					'tip' => __( 'Automatically create new terms on taxonomy field mappings and when using \'Smart Assign\'.', 'gofetch-wpjm' ),
				),
			),
		);
		...
		if ( gfjwjm_fs()->is__premium_only() ) {
			...
			$tip_geocode = __( 'Google now requires an API key for every geocoding request to their API.', 'gofetch-wpjm' ) .
							'<br/><br/>'. sprintf( __( '<a href="%s">Create/get your API key</a> and paste it here if you want to geocode feeds that contain latitudes and longitudes or geocode locations in the <code>\'location\'</code> field, on the import jobs screen.', 'gofetch-wpjm' ), 'https://developers.google.com/maps/documentation/geocoding/start#get-a-key' ).
							'<br/><br/>'. __( "Make sure you enable the <code>'Google Maps Javascript API'</code> and the <code>'Google Maps Geocoding API'</code> on your <em>Google Developers Console</em>. Otherwise you'll get javascript warnings and geocoding will fail.", 'gofetch-wpjm' );

		} else {

			$tip_geocode = sprintf( __( '<a href="%s">Create/get your API key</a> and paste it here to if you want to geocode the <code>\'location\'</code> field, on the import jobs screen.', 'gofetch-wpjm' ), 'https://developers.google.com/maps/documentation/geocoding/start#get-a-key' ) .
							'<br/><br/>'. __( "Make sure you enable the <code>'Google Maps Javascript API'</code> and the <code>'Google Maps Geocoding API'</code> on your <em>Google Developers Console</em>. Otherwise you'll get javascript warnings and geocoding will fail.", 'gofetch-wpjm' );

		}

		$geocoding_fields = array(
			array(
		...
		if ( gfjwjm_fs()->is__premium_only() ) {
			...
			$geocoding_fields[] = array(
				'title' => __( 'Rate Limit', 'gofetch-wpjm' ),
				'name'  => 'geocode_rate_limit',
				'type'  => 'text',
				'extra' => array(
					'class' => 'small-text'
				),
				'desc' => __( 'locations p/ second.', 'gofetch-wpjm' ) . ' ' . sprintf( __( 'Find more about the <a href="%s" target="_new" rel="nofollow">rate limits</a>.', 'gofetch-wpjm' ), ' https://developers.google.com/maps/documentation/geocoding/usage-limits' ),
				'tip' => __( 'This is the rate at which the importer should geocode any RSS feeds containing geo locations (latitude/longitude).', 'gofetch-wpjm' ) .
						'<br/><br/>' . __( 'Since geocoding is done using the Google maps API, the rate limit is based on your Google maps geocoding plan (<em>Standard</em> by default).', 'gofetch-wpjm' ) .
						'<br/><br/>' . __( 'Leave the default value: <code>50</code>, if you DONT\'t have a Google geocoding <em>Premium</em> plan. Leave empty to remove limits if you HAVE a Google geocoding <em>Premium</em> plan.', 'gofetch-wpjm' ) .
						'<br/><br/>' . __( 'On feeds where you are sure there are geo locations and you keep getting <em>Anywhere</em> jobs try lowering the rate limit <code>(*)</code>.', 'gofetch-wpjm' ) .
						'<br/><br/><br/>' . __( '<code>(*)</code> Note that although some feeds may contain jobs with geo locations, some jobs might not have a location. These will be defaulted to <em>Anywhere</em>. When in doubt open the feed directly on your browser, search for the job and check if it contains a location.', 'gofetch-wpjm' ),
			);
			...
			'fields' => array(
				...
				array(
					...
					'tip' => __( 'The default job duration for imported jobs.', 'gofetch-wpjm' ) .
								'<br/><br/>' . __( 'Note that this duration will only be respected if jobs are imported as "Published".', 'gofetch-wpjm' ),
				),
				array(
					...
					'tip' => __( 'Choose how the job provider source is displayed on the single job page (as set under the \'Provider Details\' group).', 'gofetch-wpjm' ) .
					'<br/><br/>' . __( 'Choosing \'None\' is not recommended. Providers should always be credited.', 'gofetch-wpjm' ),
				),
				array(
		...
		$this->tab_sections['jobs']['applications'] = array(
			...
			'fields' => array(
				array(
					...
					'type'  => 'checkbox',
					'desc' => __( 'Yes', 'gofetch-wpjm' ),
					'tip' => __( 'Check this option to use the text in the previous field, when users click on the \'Apply for Job\' button, instead of the long inline external link <code>e.g: To apply, please click here</code>.', 'gofetch-wpjm' ) .
							'<br/><br/>' . __( '(default) If this option is unchecked, the text in the previous field will be prepended to the job external link when users click on the \'Apply for Job\' button <code>e.g: To apply, please visit the following URL: https://dummyjobsite.com/job1</code>.', 'gofetch-wpjm' ),
				),
				array(
		...
		$this->tab_sections['jobs']['content'] = array(
			...
			'fields' => array(
				...
				array(
					...
					'tip' => __( 'Enable to auto format the description if it contain less than this number of paragraphs.', 'gofetch-wpjm' ) .
				 			'<br/><br/>' . __( 'This option will be ignored if the \'Format Plain Descriptions\' options is disabled.', 'gofetch-wpjm' ),
				),
				array(
					...
					'tip' => __( 'The importer will generate paragraphs on every n full stops it finds.', 'gofetch-wpjm' ) .
							'<br/><br/>' . __( 'This option will be ignored if the \'Format Plain Descriptions\' options is disabled.', 'gofetch-wpjm' ),
				),
			),
		);
		...
		$this->tab_sections['jobs']['seo'] = array(
			...
			'fields' => array(
				array(
					...
					'type'  => 'checkbox',
					'desc' => __( 'Yes', 'gofetch-wpjm' ),
					'tip' => __( 'Check this option to block search robots from indexing ALL your imported jobs pages (all providers).', 'gofetch-wpjm' ) .
							'<br/><br/>' . __( 'This option should be checked if you import jobs from providers that do not allow indexing their jobs.', 'gofetch-wpjm' ),
				),
			),
		);
		...
		$this->tab_sections['advanced']['importer'] = array(
			...
			'fields' => array(
				array(
					...
					'type'  => 'checkbox',
					'desc' => __( 'Yes', 'gofetch-wpjm' ),
					'tip' => __( 'Some RSS feeds that use <em>https://</em> might be considered invalid if your site does not use <em>https://</em>.', 'gofetch-wpjm' ) .
							 '<br/><br/>' . sprintf( __( 'Check this option to let the plugin try to load these feeds through a <a href="%s">CORS proxy</a>.', 'gofetch-wpjm' ), 'https://crossorigin.me/' ) .
							 ' ' . __( 'Leave it unchecked if you don\'t have issues loading RSS feeds.', 'gofetch-wpjm' ),
				),
			),
		);
		...
		$this->tab_sections['advanced']['log'] = array(
			...
			'fields' => array(
				array(
					...
					'tip' => __( 'Enables debug logging. Use it to report any errors to the support team. Keep it disabled, otherwise.', 'gofetch-wpjm' ) .
					'<br/><br/>' . sprintf( __( '<code>NOTE:</code> You must <a href="%s">enable</a> <code>WP_DEBUG_LOG</code> on your \'wp-config.php\' file.', 'gofetch-wpjm' ), 'https://codex.wordpress.org/Editing_wp-config.php#Configure_Error_Logging' ),
				),
				array(
					...
					'tip'   => __( 'GOFJ caches some data for faster job imports and to help find duplicates. If you delete recent jobs and immediately try to import them again, the importer will usually refuse if it finds the same jobs in cache.', 'gofetch-wpjm' ) .
					'<br/><br/>' . __( 'Check this option to clear all cached data when you click \'Save Changes\'.', 'gofetch-wpjm' ),
					'value' => 'clear_cache',
				),
			),
		);
...
?>
<style>
.<?php echo esc_attr( $this->multi_select_css_class ); ?> + .select2-container--default .select2-selection--multiple .select2-selection__choice { margin-top: 10px; }
</style>
<script type="text/javascript">
	jQuery(document).ready(function($) {
		var gofjSel2Class = '<?php echo esc_attr( $this->multi_select_css_class ); ?>';
		var options = <?php echo wp_strip_all_tags( $js_options ); ?>;
