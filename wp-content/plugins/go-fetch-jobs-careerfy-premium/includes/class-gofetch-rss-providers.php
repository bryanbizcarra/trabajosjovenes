...
class GoFetch_RSS_Providers {
		...
		$providers = array(
			...
			'jobs.theguardian.com' => array(
				...
				'feed' => array(
					...
					'regexp_mappings' => array(
						...
						'salary'  => '/^(.*?):/is',    // e.g: £27,499 - £33,569: Google:
					),
					// Feed URL query args.
					'query_args'  => array(
			...
			'jobs.marketinghire.com' => array(
				...
				'feed' => array(
					...
					'regexp_mappings' => array(
						'title' => array(
							'company'  => '/.*\\|(.*)/', // e.g: Developer | Google
						),
						'description' => array(
							'location' => '/^(.*?),/is', // e.g: Lisbon, etc...
						),
					),
					'regexp_title' => '/(.*)\\|/', // e.g: Developer | Google
					// Feed URL query args.
					'query_args'  => array(
						'keyword'  => array( 'keywords' => '' ),
	...
	public static function setup_instructions_for( $provider ) {
		...
		if ( ! empty( $data['feed']['base_url'] ) && ! empty( $data['logo'] ) ) {
			...
			$atts = array( 'src' => esc_url( $data['logo'] ), 'class' => 'provider-logo-orig' );

			$header  = html( 'a', array( 'href' => esc_url( $data['website'] ), 'target' => '_blank' ), html( 'img', $atts ) );
		...
		} else if ( ! empty( $data['API']['required_fields'] ) ) {
		...
		switch ( $provider ) {
			...
			case __( 'Other', 'gofetch-wpjm' ):
				...
				$setup .= '<br/>' . html( 'p', __( 'In any case, most job sites usually offer a pre-set list of RSS feeds or a custom RSS builder based on a job search.', 'gofetch-wpjm' ) .
				' ' . __( 'Just follow the instructions for similar providers and you should be ready to go.', 'gofetch-wpjm' ) );
				break;

			default:

				if ( 'ATS' !== $data['category'] && 'API' !== $data['category'] ) {
		...
		if ( ! empty( $data['feed']['default'] ) ) {
			$data['feed']['fixed'] = array( __( 'Latest Jobs', 'gofetch-wpjm' ) => $data['feed']['base_url'] );
		}
		...
		if ( empty( $data['feed']['full_description'] ) ) {
		...
		if ( 'ATS' !== $data['category'] && 'API' !== $data['category'] ) {
			...
			$manual_setup = html( 'p', html( 'a', array( 'href' => '#', 'class' => 'provider-expand-feed-manual-setup', 'data-child' => 'feed-manual-setup', 'data-default' => __( 'View Manual Setup Instructions', 'gofetch-wpjm' ) ), __( 'View Manual Setup Instructions', 'gofetch-wpjm' ) ) );
		...
		if ( ! empty( $data['feed']['query_args'] ) ) {
			...
			if ( gfjwjm_fs()->can_use_premium_code() ) {
				$feed_builder = html( 'p', html( 'a', array( 'href' => '#', 'class' => 'provider-expand-feed-builder', 'data-child' => 'feed-builder', 'data-default' => __( 'Use Feed Builder', 'gofetch-wpjm' ) ), __( 'Use RSS Feed Builder', 'gofetch-wpjm' ) ) );
				$setup .= $feed_builder . html( 'div class="feed-builder"', self::output_rss_feed_builder( $provider, $data ) );
			} else {
				$feed_builder = html( 'p', html( 'a', array( 'href' => '#', 'class' => 'provider-expand-feed-builder-example', 'data-child' => 'feed-builder', 'data-default' => __( 'Feed Builder (non-working example)', 'gofetch-wpjm' ) ), GoFetch_Admin::limited_plan_warn() ) );
				$setup .= $feed_builder . html( 'div class="feed-builder"', self::output_rss_feed_builder( $provider, $data ) );
			}
		}
	...
	public static function output_providers_dropdown( $atts = array() ) {
		...
		foreach ( $choices_n as $providers ) {
			...
			foreach ( $providers as $provider ) {
				...
				foreach ( $cats as $category ) {
					$categories[ $category ][ $provider ] = sprintf( '%1$s %2$s %3$s', ( isset( $data['API'] ) ? '<em>[API]</em>' : '' ), ( isset( $data['multi_region_match'] ) || isset( $data['multi-region'] ) || isset( $data['region_domains'] ) ? __( '<em>[Multi-Region]</em>', 'gofetch-wpjm' ) : '' ), ( ! empty( $data['description'] ) ? $data['description'] : '' ) );
				}
			}
		}
		...
		foreach ( $categories as $category => $providers ) {
			...
			$optgroup .= html( 'optgroup', array( 'label' => esc_attr( $category ) ), $options );
			$options  = '';

		}

		$optgroup = html( 'option', array( 'value' => '' ), __( 'Choose a Job Provider . . .', 'gofetch-wpjm' ) ) . $optgroup;
		...
		$html .= '<br/>' . html( 'div class="secondary-container"', '<span class="dashicons-before dashicons-info"></span>' .
		html( 'div', sprintf( 'Please note that <em>%s</em> cannot guarantee that all the providers will keep their RSS feeds available forever.<br/>Consider using a secondary RSS feed provider to avoid being dependent on a single one.', 'Go Fetch Jobs' ) ) );
...
?>
		<?php if ( ! $limited ) : ?>
			...
			<p><?php echo __( 'This is a basic builder to help you setup a customized feed. If you need to further refine the feed please read the manual setup instructions.', 'gofetch-wphm' ); ?></p><br/>

		<?php else: ?>

			<br><?php echo sprintf( __( 'This is a non working feed builder. Full functionality is available on <a href="%s">premium plans</a> only. Fields displayed will vary depending on the provider.</br> The <em>Free</em> version only provides a fixed URL example that you can copy&paste, below.', 'gofetch-wphm' ), esc_url( gfjwjm_fs()->get_upgrade_url() ) ); ?></p>
		...
		<p>
			...
			if ( isset( $data['region_domains'] ) ) {
				...
				}  else if ( ! empty( $data['region_default'] ) ) {
					$default_domain = $data['region_default'];
				}

				?><label for="feed-domain"><strong><?php _e( 'Feed Country/State', 'gofetch-wpjm' ); ?></strong></label><?php
			...
			<label for="feed-url"><strong><?php _e( 'Feed Base URL', 'gofetch-wpjm' ); ?></strong></label>
			<input <?php disabled( $limited ); ?> type="text" class="regular-text2" name="feed-url"><a href="#" <?php disabled( $limited ); ?> class="button secondary reset-feed-url" title="<?php echo __( 'Reset to the original Feed URL.', 'gofetch-wpjm' ) ?>"><?php echo __( 'Reset', 'gofetch-wpjm' ) ?></a>
		</p>

		<br/>

		<p class="params opt-param-keyword">
			<label for="feed-keyword"><strong><?php _e( 'Keyword', 'gofetch-wpjm' ); ?></strong></label><span class="feed-param-keyword"></span>
			<input <?php disabled( $limited ); ?> type="text" class="regular-text" name="feed-keyword" data-qarg="feed-param-keyword" placeholder="<?php echo __( 'e.g.: design, writer, doctor, etc', 'gofetch-wpjm' ); ?>">
		</p>

		<p class="params opt-param-location">
			<label for="feed-location"><strong><?php _e( 'Location', 'gofetch-wpjm' ); ?></strong></label><span class="feed-param-location"></span>
			<input <?php disabled( $limited ); ?> type="text" class="regular-text" name="feed-location" data-qarg="feed-param-location" placeholder="<?php echo __( 'e.g.: new york, lisbon, london, etc', 'gofetch-wpjm' ); ?>">
		</p>

		<p class="params opt-param-radius">
			<label for="feed-radius"><strong><?php _e( 'Radius', 'gofetch-wpjm' ); ?></strong></label><span class="feed-param-radius"></span>
			<input <?php disabled( $limited ); ?> type="text" class="regular-text" style="width: 100px" style name="feed-radius" data-qarg="feed-param-radius" placeholder="<?php echo __( 'e.g.: 100', 'gofetch-wpjm' ); ?>">
		</p>

		<div class="clear"></div>

		<p class="params opt-param-state">
			<label for="feed-state"><strong><?php _e( 'State', 'gofetch-wpjm' ); ?></strong></label><span class="feed-param-state"></span>
			<input <?php disabled( $limited ); ?> type="text" class="regular-text" name="feed-state" data-qarg="feed-param-state" placeholder="<?php echo __( 'e.g.: new york, florida, etc', 'gofetch-wpjm' ); ?>">
			</p>

		<p class="params opt-param-type">
			<label for="feed-type"><strong><?php _e( 'Job Type', 'gofetch-wpjm' ); ?></strong></label> <span class="feed-param-type"></span>
			<input <?php disabled( $limited ); ?> type="text" class="regular-text" name="feed-type" data-qarg="feed-param-type" placeholder="<?php echo __( 'e.g.: fulltime, freelance', 'gofetch-wpjm' ); ?>">
		</p>

		<p class="params opt-param-category">
			<label for="feed-category"><strong><?php _e( 'Job Category', 'gofetch-wpjm' ); ?></strong></label> <span class="feed-param-category"></span>
			<input <?php disabled( $limited ); ?> type="text" class="regular-text" name="feed-category" data-qarg="feed-param-category" placeholder="<?php echo __( 'e.g.: writer, design', 'gofetch-wpjm' ); ?>">
		</p>

		<?php do_action( 'goft_wpjm_feed_builder_fields', $provider, $data ); ?>

		<?php $default_limit = ! empty( $data['feed']['query_args']['limit']['default'] ) ? $data['feed']['query_args']['limit']['default'] : ''; ?>

		<p class="params opt-param-limit">
			<label for="feed-limit"><strong><?php _e( 'Limit', 'gofetch-wpjm' ); ?></strong>
				<span class="tip"><span class="dashicons-before dashicons-editor-help tip-icon bc-tip" data-tooltip="<?php echo esc_attr( __( 'Number of offers to retrieve.', 'gofetch-wpjm' ) ); ?>"></span></span>
			</label><span class="feed-param-limit"></span>
			<input <?php disabled( $limited ); ?> type="text" class="regular-text" value="<?php echo esc_attr( $default_limit ); ?>" style="width: 100px" style name="feed-limit" data-qarg="feed-param-limit" placeholder="<?php echo __( 'e.g.: 50', 'gofetch-wpjm' ); ?>">
		</p>

		<?php if ( ! empty( $data['feed']['pagination'] ) ) : ?>
			<div class="clear"></div>

			<div class="provider-data secondary-container">
				<span class="dashicons-before dashicons-info"></span>
				<div>
					<?php echo __( '<strong>This feed supports pagination!</strong> Although not recommended, you can import more jobs than the default limit.', 'gofetch-wpjm' ); ?>
					<br/>
					<?php echo __( 'Please note that the higher the limit the slower the import process will be.', 'gofetch-wpjm' ); ?>
				</div>
		</div>
		<?php endif; ?>
		...
		<?php if ( gfjwjm_fs()->is__premium_only() ) : ?>

			<?php if ( GoFetch_Helper::supports_scraping( $data ) ) : ?>

				<div class="provider-data secondary-container">
					<span class="dashicons-before dashicons-info"></span>
					<?php
						echo html( 'div', sprintf( __( '<strong>This feed supports scraping* for: </strong> %s.', 'gofetch-wpjm' ), implode( ', ', array_keys( $data['special']['scrape'] ) ) ) .
									__( '<br/><small>(*) <b>Scraping</b> is the ability to extract additional metadata directly from the provider website</small>', 'gofetch-wpjm' ) );
					?>
				</div>
		...
		<?php if ( ( isset( $data['feed']['scraping'] ) && ! $data['feed']['scraping'] ) || empty( $data['feed']['full_description'] ) ) : ?>
		...
		<?php if ( ! empty( $data['partner'] ) ) : ?>
			...
			<div class="provider-data secondary-container partner revenue-share">
				...
				<div>
					<?php echo wp_kses_post( $data['partner_msg'] ); ?>
				</div>
		</div>
		<?php endif; ?>

		<?php echo self::feed_placeholder( $limited ); ?>
		...
		<?php if ( ! $limited ) : ?>
			...
			<input type="hidden" name="feed-param-split-multi" value="<?php echo (int) ! empty( $data['feed']['split_params'] ); ?>">
...
?>
		<p class="provider-rss-custom-placeholder">
			<label for="provider-rss-custom-placeholder">
				<?php if ( ! $limited ) : ?>
					<span class="dashicons dashicons-rss"></span><strong><?php _e( 'Your Custom Feed', 'gofetch-wpjm' ); ?></strong>
					<?php echo self::copy_paste(); ?>
				<?php elseif ( ! empty( $data['feed']['example'] ) ) : ?>
					<span class="dashicons dashicons-rss"></span><strong><?php _e( 'Your RSS Feed', 'gofetch-wpjm' ); ?></strong>
					<?php
...
<?php
	...
	public static function find_provider_in_url( $url, $provider_match = '' ) {
		...
		foreach ( $providers as $provider_id => $meta ) {
			...
			if ( isset( $meta['feed']['url_match'] ) ) {
				...
				if ( $match )  return $provider_id;

			} elseif ( false !== strpos( $url, $meta['feed']['base_url'] ) || ( ! empty( $url_parts['host'] ) && false !== strpos( $url, $url_parts['host'] ) ) ) {
				return $provider_id;
			}
		}
		return apply_filters( 'goft_wpjm_provider_in_url', false, $url, $provider_match );
	}
	...
	public static function get_providers_list( $category = '' ) {
		...
		foreach ( $providers as $provider => $data ) {
			...
			$meta[ strtolower( $data['category'] ) ][ $free ][] = $provider;
		}
