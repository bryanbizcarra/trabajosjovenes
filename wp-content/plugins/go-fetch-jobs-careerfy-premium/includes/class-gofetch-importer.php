...
class GoFetch_Importer {
	...
	public function __construct() {
		...
		add_filter( 'goft_wpjm_prepare_item', array( __CLASS__, 'get_item_logo' ), 12, 2 );
	...
	public static function get_item_logo( $item, $params ) {
		...
		if ( ! empty( $item['logo'] ) || empty( $params['logos'] ) ) {
			...
			if ( ! empty( $item['logo'] ) && empty( $item['logo_html'] ) ) {
				$item['logo_html'] = html( 'img', array( 'src' => $item['logo'], 'class' => 'goft-og-image' ) );
			}
			return $item;
		}
		...
		if ( ! empty( $og ) && empty( $item['logo'] ) ) {
			$image = $og->image;

			if ( $image ) {
				$item['logo']      = $image;
				$item['logo_html'] = html( 'img', array( 'src' => $image, 'class' => 'goft-og-image' ) );
			}
		}

		// Clear memory.
		$item['try_og_logo'] = null; unset( $item['try_og_logo'] );
	...
	public static function import_custom_content_type( $url_or_data, $is_file_upload = false, $retries = 0 ) {
		...
		if ( GoFetch_Helper::is_json( $response ) ) {
			...
			$items = GoFetch_Helper::xml_to_array_parse( $xml );

			$is_job_feed = ! empty( $xml->job ) || ! empty( $xml->item ) || ! empty( $xml->channel->item );

			if ( ! empty( $items ) ) {
				$feed['items'] = GoFetch_Helper::remove_xml_invalid_nodes( $items, $is_job_feed );
			}

			// If we don't have a simple array parse it using a different strategy.
			if ( ( $xml && empty( $feed['items'][0]['title'] ) ) || ! count( $feed['items'] ) || 1 === count( $feed['items'] ) ) {
