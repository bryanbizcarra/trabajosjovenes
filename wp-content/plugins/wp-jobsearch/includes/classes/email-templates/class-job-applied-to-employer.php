            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                if (isset($user->ID) && $user->ID > 0) {
                    ...
                    if ($is_candidate) {
                        ...
                        // for cover letter
                        $get_job_apps_cver_att = get_post_meta($job_id, 'job_apps_cover_attachs', true);
                        $get_job_apps_cver_att = !empty($get_job_apps_cver_att) ? $get_job_apps_cver_att : array();
                        $_job_apps_cver = isset($get_job_apps_cver_att[$_candidate_id]) ? $get_job_apps_cver_att[$_candidate_id] : '';
                        if ($_job_apps_cver) {
                            $cand_cover_file = get_post_meta($_candidate_id, 'candidate_cover_letter_file', true);
                            $covr_file_attach_id = isset($cand_cover_file['file_id']) ? $cand_cover_file['file_id'] : '';
                            $file_url = isset($cand_cover_file['file_url']) ? $cand_cover_file['file_url'] : '';
                            $file_mimetype = isset($cand_cover_file['mime_type']) ? $cand_cover_file['mime_type'] : '';
                            if ($file_mimetype != '') {
                                $covrsub_file_url = substr($file_url, strpos($file_url, 'jobsearch-resumes/'), strlen($file_url));
                                $covr_file_path = $uplod_dirpath . '/' . $covrsub_file_url;
                            }
                        }
                        // cover letter end
                ...
                if ($attach_cover_file != 'off' && $covr_file_path != '') {
                    $args['att_file_path'] = array($covr_file_path);
                }
                ...
                if ($attach_resume_file != 'off') {
                    if (isset($att_file_id) && is_numeric($att_file_id) && $att_file_id > 0 && get_post_type($att_file_id) == 'attachment') {
                        ...
                        if ($att_file_path != '') {
                            if (is_array($args['att_file_path'])) {
