...
class JobSearch_plugin
{
    ...
    private function load_dependencies()
    {
        ...
        if (!class_exists('Careerfy_framework')) {
            $dt7_theme = wp_get_theme('dt-the7');
            $nanosoft_theme = wp_get_theme('nanosoft');
            $theme_2121 = wp_get_theme('theme2121');
            $theme_pylon = wp_get_theme('pylon');
            $theme_finlab = wp_get_theme('finlab');
            $theme_mf = wp_get_theme('mf');
            if (!class_exists('TGM_Plugin_Activation') && !$dt7_theme->exists() && !$nanosoft_theme->exists() && !$theme_2121->exists() && !$theme_pylon->exists() && !$theme_finlab->exists() && !$theme_mf->exists()) {
