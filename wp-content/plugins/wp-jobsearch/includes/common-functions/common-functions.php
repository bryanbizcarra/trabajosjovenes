...
function jobsearch_custom_fields_options_slugtolabel($field_value, $saved_field_data) {
    ...
    foreach ($saved_field_data['options']['value'] as $key => $cus_field_options_value) {
        ...
        $drop_down_arr[$cus_field_options_value] = (apply_filters('wpml_translate_single_string', $saved_field_data['options']['label'][$cut_field_flag], 'Custom Fields', 'Dropdown Option Label - ' . $saved_field_data['options']['label'][$cut_field_flag], $lang_code));
        $cut_field_flag++;
    }
