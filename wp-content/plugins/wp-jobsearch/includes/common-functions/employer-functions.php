                ...
                if ($download_cv == '1') {
                    ...
                    if ($multiple_cv_files_allow == 'on') {
                        ...
                        if (!empty($ca_at_cv_files)) {
                            ...
                            $downloadcv_link_btn = apply_filters('jobsearch_candetail_page_cvdownload_link', $downloadcv_link_btn, $candidate_id);
                        }
    ...
    if (!empty($candidate_cv_file_att)) {
        ...
        <?php } else {
            ...
            if (in_array('administrator', (array)$cur_user_obj->roles)) {
                ...
                $free_shortlist_allow = apply_filters('jobsearch_cand_free_save_switch_val', $free_shortlist_allow);
                if ((in_array($candidate_id, $employer_resumes_list) || ($free_shortlist_allow == 'on')) && apply_filters('jobsearch_allow_free_cv_download_check', true, $user_id, $candidate_id)) {
                    echo ($download_link_btn);
                } else {
                    ob_start();
                    if ($view == 'cand5') { ?>
...
function jobsearch_overallpckg_order_used_invites($order_id = 0)
{
    $cvs_list_count = 0;
    if ($order_id > 0) {
        $cvs_list = get_post_meta($order_id, 'jobsearch_order_invites_list', true);
...
function jobsearch_overallpckg_order_remaining_invites($order_id = 0)
{
    $remaining_cvs = 0;
    if ($order_id > 0) {
        $total_cvs = get_post_meta($order_id, 'num_of_invites', true);
